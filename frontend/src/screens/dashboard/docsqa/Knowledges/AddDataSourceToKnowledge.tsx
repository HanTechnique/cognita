...
const AddDataSourceToKnowledge = ({
  knowledgeName,
  open,
  onClose,
}: NewKnowledgeProps) => {
  ...
  const [addDocsToKnowledge] = useAddDocsToKnowledgeMutation()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addDocsToKnowledge(addDocsParams).unwrap()
      ...
      notify(
        ...
        'Updated knowledge will be available to use after 3-5 minutes.'
      )
    } catch (err: any) {
      notify(
        'error',
        'Failed to add documents to knowledge!',
        err?.error ||
          err?.details?.msg ||
          err?.message ||
          'There was an error while adding documents to knowledge.'
      )
    }
    setIsSaving(false)
  }
...
export default AddDataSourceToKnowledge
