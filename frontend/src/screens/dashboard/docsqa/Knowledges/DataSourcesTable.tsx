...
const DataSourceDeleteButton = ({
  ...
  const handleSubmit = async () => {
    ...
    notify(
      'success',
      'Data Source is successfully detached from the Knowledge!',
      'Updated kownledge will be available to use after 3-5 minutes.'
    )
  }
...
const DataSourcesTable = ({
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'fqn', headerName: 'FQN', flex: 1 },
    {
      ...
      flex: 1,
      renderCell: (params: GridRenderCellParams) => {
        const key = knowledgeName + ':' + params?.row?.fqn
        return (
