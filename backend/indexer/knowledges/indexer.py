...
from backend.indexer.knowledges.types import KnowledgeDataIngestionConfig
from backend.logger import logger
from backend.indexer.indexer import get_data_point_fqn_to_hash_map
...
from backend.types.core import Knowledge
...
async def sync_data_source_to_knowledge(inputs: KnowledgeDataIngestionConfig):
    """
    Synchronizes the data source to the knowledge by performing the following steps:
    ...
    3. Logs the total number of existing data point vectors in the knowledge.
    4. Updates the data ingestion run status to indicate that data ingestion has started.
    5. Calls the _sync_data_source_to_knowledge function to perform the actual data ingestion.
    ...
    Args:
        inputs (KnowledgeDataIngestionConfig): The configuration for data ingestion.
    ...
    try:
        existing_data_point_vectors = GRAPHRAG_STORE_CLIENT.list_data_point_vectors(
            knowledge_name=inputs.knowledge_name,
        ...
        logger.info(
            f"Total existing data point vectors in collection {inputs.knowledge_name}: {len(existing_data_point_vectors)}"
        )
    ...
    if inputs.data_ingestion_mode == DataIngestionMode.FULL:
        ...
        try:
            GRAPHRAG_STORE_CLIENT.delete_data_point_vectors(
                knowledge_name=inputs.knowledge_name,
                data_point_vectors=existing_data_point_vectors,
            )
        except Exception as e:
...
async def _sync_data_source_to_collection(
    inputs: KnowledgeDataIngestionConfig, previous_snapshot: Dict[str, str] = None
):
    """
...
async def ingest_data_points(
    inputs: KnowledgeDataIngestionConfig,
    ...
    GRAPHRAG_STORE_CLIENT.upsert_documents(
        knowledge_name=inputs.knowledge_name,
...
async def ingest_data(
    ...
    try:
        ...
        if not isinstance(knowledge, Knowledge):
            knowledge = Knowledge(**knowledge.model_dump())

        if not knowledge:
            logger.error(
                f"Knowledge with name {request.knowledge_name} does not exist."
            )
            raise HTTPException(
                status_code=404,
                detail=f"Knowledge with name {request.knowledge_name} does not exist.",
            )

        if not knowledge.associated_data_sources:
            logger.error(
                f"Collection {request.knowledge_name} does not have any associated data sources."
            )
            raise HTTPException(
                status_code=400,
                detail=f"Collection {request.knowledge_name} does not have any associated data sources.",
            )
        associated_data_sources_to_be_ingested = []
        if request.data_source_fqn:
            associated_data_sources_to_be_ingested = [
                knowledge.associated_data_sources.get(request.data_source_fqn)
            ]
        else:
            associated_data_sources_to_be_ingested = (
                knowledge.associated_data_sources.values()
            )
        ...
        for associated_data_source in associated_data_sources_to_be_ingested:
            ...
            if not request.run_as_job or settings.LOCAL:
                ...
                ingestion_config = KnowledgeDataIngestionConfig(
                ...
                if pool:
                    logger.info(
                        f"Submitting sync_data_source_to_knowledge job to pool"
                    )
                    # future of this submission is ignored, failures not tracked
                    pool.submit(sync_data_source_to_knowledge, ingestion_config)
                else:
                    logger.info(
                        f"Running sync_data_source_to_knowledge on main event loop"
                    )
                    await sync_data_source_to_knowledge(ingestion_config)
