...
if TYPE_CHECKING:
    from backend.types.core import Collection, Knowledge  # Import for type hinting
...
class BaseKnowledge(ConfiguredBaseModel):
    """
    Base knowledge configuration
    """

    name: Annotated[str, StringConstraints(pattern=r"^[a-z][a-z0-9-]*$")] = Field(  # type: ignore
        title="a unique name to your knowledge",
        description="Should only contain lowercase alphanumeric character and hypen, should start with alphabet",
        example="test-knowledge",
    )
    description: Optional[str] = Field(
        None,
...
class Collection(BaseCollection):
    ...
    def ensure_associated_data_sources_not_none(
        ...
        if values.get("knowledges") is None:
            values["knowledges"] = []
        return values

class Knowledge(BaseKnowledge):
    id: int  = Field(
        title="Id for the knowledge"
    )
    ...
    def ensure_associated_data_sources_not_none(
        ...
        if values.get("associated_data_sources") is None:
            values["associated_data_sources"] = {}
        if values.get("collections") is None:
            values["collections"] = []
        return values
